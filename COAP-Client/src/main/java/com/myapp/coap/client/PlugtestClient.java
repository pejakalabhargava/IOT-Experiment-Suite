...
public class PlugtestClient {
	...
	public static void main(String[] args) {
		...
		if (args.length == 0) {
			...
			System.out.println("Usage: " + PlugtestClient.class.getSimpleName()
					+ " [-s] URI");
			System.out
					.println("  -s        : Skip the ping in case the remote does not implement it");
			System.out
		...
		if (first == 1) {
			...
			if (!clientPing.ping(2000)) {
				System.out.println(uri
						+ " does not respond to ping, exiting...");
				System.exit(-1);
			} else {
				...
				EndpointManager.getEndpointManager().getDefaultEndpoint()
						.addInterceptor(new MessageTracer());
			}
		}
		/* TASK 2: for e2e delay; add timer here!!-> will get 1RTT */
		testCC(uri);
		/* end task2 time here */
		// testCB(uri);
		// testCO(uri);
		// testCL(uri);
		/* NOTE: Also see largeRequestPayload at the end of this file */
	...
	public static void testCC(String uri) {
		...
		System.out.println(response.advanced().getType() + "-"
				+ response.getCode());
		System.out.println(response.getResponseText());
		/* Task 1: calculate end value */
		/*
		 * System.out.println("===============\nCC02");
		 * System.out.println("---------------\nDELETE /test\n---------------");
		 * response = client.delete();
		 * System.out.println(response.advanced().getType() + "-" +
		 * response.getCode()); System.out.println(response.getResponseText());
		 * 
		 * System.out.println("===============\nCC03");
		 * System.out.println("---------------\nPUT /test\n---------------");
		 ...
		 */
		/*
		 * client.setURI(uri + "/separate"); client.useCONs();
		 * 
		 * System.out.println("===============\nCC09+11");
		 * System.out.println("---------------\nGET /separate\n---------------"
		 * ); response = client.get();
		 ...
		 * 
		 * System.out.println("===============\nCC12"); System.out.println(
		 * "---------------\nGET /test w/o Token\n---------------"); Request
		 * req12 = Request.newGet(); // never re-use a Request object
		 * req12.setToken(new byte[0]); response = client.advanced(req12);
		 * System.out.println(response.advanced().getType() + "-" +
		 * response.getCode()); System.out.println(response.getResponseText());
		 * 
		 * client.setURI(uri + "/seg1/seg2/seg3"); /*just gets a bigger
		 * segment!!!
		 */
		/*
		 * System.out.println("===============\nCC13"); System.out.println(
		 * "---------------\nGET /seg1/seg2/seg3\n---------------"); response =
		 * client.get(); System.out.println(response.advanced().getType() + "-"
		 * + response.getCode());
		 * System.out.println(response.getResponseText());
		 * 
		 * client.setURI(uri + "/query?first=1&second=2");
		 * 
		 * System.out.println("===============\nCC14"); System.out.println(
		 * "---------------\nGET /query?first=1&second=2\n---------------");
		 * response = client.get();
		 * System.out.println(response.advanced().getType() + "-" +
		 * response.getCode()); System.out.println(response.getResponseText());
		 * 
		 * client.setURI(uri + "/separate"); client.setTimeout(10000);
		 * client.useNONs();
		 * 
		 * System.out.println("===============\nCC17");
		 * System.out.println("---------------\nNON-GET /separate\n---------------"
		 ...
		 * response.getCode()); System.out.println(response.getResponseText());
		 * 
		 * client.setURI(uri + "/test"); client.setTimeout(0); client.useCONs();
		 * 
		 * System.out.println("===============\nCC18"); System.out.println(
		 * "---------------\nPOST /test for Location-Path\n---------------");
		 * response = client.post("TD_COAP_CORE_18",
		 * MediaTypeRegistry.TEXT_PLAIN); System.out.println(response.getCode()
		 * + "-" + response.getOptions().getLocationString());
		 * System.out.println(response.getResponseText());
		 * 
		 * client.setURI(uri + "/location-query");
		 * 
		 * System.out.println("===============\nCC19"); System.out.println(
		 * "---------------\nGET /location-query\n---------------"); response =
		 ...
		 * System.out.println(response.getResponseText());
		 * 
		 * client.setURI(uri + "/validate"); byte[] etag;
		 * 
		 * System.out.println("===============\nCC21");
		 * System.out.println("---------------\nGET /validate\n---------------"
		 * ); response = client.get(); if
		 * (response.getOptions().getETagCount()==1) { etag =
		 * response.getOptions().getETags().get(0);
		 * System.out.println(response.getCode() + " - ETag [" +
		 * Utils.toHexString(etag) + "]");
		 * System.out.println(response.getResponseText());
		 * 
		 * System.out.println(
		 * "---------------\nGET /validate with ETag\n---------------");
		 * response = client.validate(etag); etag =
		 * response.getOptions().getETags().get(0);
		 * System.out.println(response.getCode() + " - ETag [" +
		 * Utils.toHexString(etag) + "]");
		 * System.out.println(response.getResponseText());
		 ...
		 * response = clientStimulus.put("CC21 at " + new
		 ...
		 * System.out.println(response.getResponseText());
		 * 
		 * } else { System.out.println("Error - no ETag"); }
		 */
		/*
		 * System.out.println("===============\nCC22"); System.out.println(
		 * "---------------\nGET /validate with If-Match\n---------------");
		 * response = client.get(); if (response.getOptions().getETagCount()==1)
		 * { etag = response.getOptions().getETags().get(0);
		 * System.out.println(response.getCode() + " - ETag [" +
		 * Utils.toHexString(etag) + "]");
		 * System.out.println(response.getResponseText());
		 * 
		 * System.out.println("---------------\nPUT /validate If-Match [" +
		 * Utils.toHexString(etag) + "]\n---------------"); response =
		 * client.putIfMatch("CC22 at " + new
		 * SimpleDateFormat("HH:mm:ss.SSS").format(new Date()),
		 ...
		 * "---------------\nPUT /validate stimulus\n---------------");
		 * CoapClient clientStimulus = new CoapClient(uri + "/validate");
		 * response = clientStimulus.put("CC22 at " + new
		 * SimpleDateFormat("HH:mm:ss.SSS").format(new Date()),
		 * MediaTypeRegistry.TEXT_PLAIN);
		 * System.out.println(response.getCode());
		 * 
		 ...
		 * 
		 * System.out.println("===============\nCC23"); System.out.println(
		 * "---------------\nPUT /create1 with If-None-Match\n---------------");
		 * response = client.putIfNoneMatch("CC23 at " + new
		 * SimpleDateFormat("HH:mm:ss.SSS").format(new Date()),
		 * MediaTypeRegistry.TEXT_PLAIN);
	...
	public static void testCB(String uri) {
		...
		System.out
				.println("---------------\nPUT /large-update\n---------------");
		response = client.put(getLargeRequestPayload(),
				MediaTypeRegistry.TEXT_PLAIN);
		System.out.println(response.getCode());
		System.out.println(response.getResponseText());
	...
	public static void testCO(String uri) {
		...
		System.out.println("===============\nCO04");
		System.out.println("---------------\nGET /obs with Observe");
		CoapObserveRelation relation4 = client
	...
	public static void testCL(String uri) {
		...
		for (WebLink link : links) {
			...
			System.out.println("---------------\nGET " + link.getURI()
					+ " with ct=40\n---------------");
			Set<WebLink> links2 = LinkFormat.parse(client.get(
